...
try:
    ...
    from .ui_specs import RezoUISpec
    from .ui_common import (
        BASE_TARGET_NAMES, COMMON_PAGE_TITLES, DAMP_NAMES, LAYOUT_NAMES,
        PALETTE_NAMES, ROW_DRY_NAMES, SAVE_NAMES, format_frequency_name,
...
class RezoCore(RezoCoreConstants, wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        # The SVF uses inverse-Q: lower values are more resonant. DAMP reduces
        # the requested resonance amount as feedback rises instead of clamping
        # inverse-Q to a floor. Every mode therefore retains a useful RES range
        # all the way to its upper endpoint.
        resonance_ctl = Signal(ASQ)
        feedback_damp = Signal(unsigned(16))
        m.d.comb += [
            feedback_damp.eq(feedback_damping(
                effective_feedback, self.damp_mode, "rezo")),
...
class RezoTileDisplay(wiring.Component):
    ...
    def elaborate(self, platform):
        ...
        # Keep all family text stores packed. The live row address is computed
        # one pixel early below, which removes the multiplier from the BRAM
        # setup path without spending block RAM on sparse 64x64 pages.
        text_row_stride = 45
        ...
        compact_matrix_text_rows = (17, 21, 25, 29, 33)
        ...
        put_native(8, "IN 1", 18, 15)
        put_native(8, "IN 2", 25, 15)
        put_native(8, "IN 3", 32, 15)
        ...
        m.d.comb += [
            ...
            feedback_selected_valid.eq(
                (selected_sync >= RezoUISpec.TARGET_FEEDBACK_SEND_BASE) &
                (selected_sync < RezoUISpec.TARGET_FEEDBACK_SEND_BASE +
                 RezoCore.N_BANDS)),
        ]
        ...
        m.d.comb += [
            ...
            bands_selected_valid.eq(
                ((selected_sync >= RezoUISpec.TARGET_BAND_ENABLE_BASE) &
                 (selected_sync < RezoUISpec.TARGET_BAND_ENABLE_BASE +
                  RezoCore.N_BANDS)) |
                ((selected_sync >= RezoUISpec.TARGET_BAND_FREQ_BASE) &
                 (selected_sync < RezoUISpec.TARGET_BAND_FREQ_BASE +
                  RezoCore.N_BANDS))),
        ...
        # writer-address layout below index 96. Indices 104..110 hold the
        # fixed-width ROW DRY option value.
        update_index = Signal(range(111))
        ...
        m.d.comb += [
            writer_char.eq(0),
            selected_band.eq(0),
            selected_band_valid.eq((selected_sync >= RezoUISpec.TARGET_BAND_BASE) &
                                   (selected_sync < RezoUISpec.TARGET_BAND_BASE +
                                    RezoCore.N_BANDS)),
        ...
        m.d.comb += displayed_layout.eq(Mux(
            editing_sync & (selected_sync == RezoUISpec.TARGET_BAND_LAYOUT),
        ...
        row_dry_chars = [Array(Const(self.code(name[pos]), 6)
                               for name in ROW_DRY_NAMES)
                         for pos in range(7)]
        ...
        for n, (_, _, depth_row) in enumerate(compact_input_text_rows):
            for pos in range(5):
                writer_address_init[76 + n * 5 + pos] = native_text_address(
                    2, 13, depth_row, pos)
        for pos in range(7):
            writer_address_init[104 + pos] = native_text_address(
                5, 22, 20, pos)
        ...
        with m.Switch(update_index):
            ...
            for pos in range(7):
                with m.Case(104 + pos):
                    m.d.comb += writer_char.eq(
                        row_dry_chars[pos][row_dry_sync])
        writer_last_index = 110
        ...
        m.d.comb += [
            ...
            # Circle intersections are symmetric in x/y. A second port on
            # the same DP16KD set supplies the bottom arc without another ROM.
        ...
        # after VALUE summing/clamping and before DRIVE or feedback. Reuse the
        # outer output-meter annulus, transposed through circle symmetry, and
        # fill it left-to-right on the same calibrated -60..0 dBFS scale.
        ...
        # A persistent radial bar marks nominal 0 dB (5 V peak). The remaining
        # 4.3 dB up to the 8.192 V ADC ceiling is visible headroom.
        ...
        content_y1 = Signal(unsigned(10), init=NATIVE_CONTENT_PANEL_Y1)
        m.d.dvi += [
        ...
        control_panel_x0 = 283
        control_panel_x1 = 594
        control_fill_x0 = 289
        ...
        tune_panel_x0 = NATIVE_FEEDBACK_TRACK_X0
        tune_panel_x1 = NATIVE_FEEDBACK_TRACK_X1
        ...
        palette_chip = advanced_page & self.rect(
            text_x, text_y, native_value_chip_x0(22), 244, 456, 284)
        palette_select = advanced_page & (
            selected_dvi_q == RezoUISpec.TARGET_PALETTE) & self.outline(
                text_x, text_y, native_value_chip_x0(22) - 4,
                240, 460, 288, t=3)
        save_default_chip = advanced_page & self.rect(
            text_x, text_y, native_value_chip_x0(22), 372, 472, 412)
        save_default_select = advanced_page & (
            selected_dvi_q == RezoUISpec.TARGET_SAVE_DEFAULT) & self.outline(
                text_x, text_y, native_value_chip_x0(22) - 4,
                368, 476, 416, t=3)
        row_dry_chip = advanced_page & self.rect(
            text_x, text_y, native_value_chip_x0(22), 308, 472, 348)
        row_dry_select = advanced_page & (
            selected_dvi_q == RezoUISpec.TARGET_ROW_DRY) & self.outline(
                text_x, text_y, native_value_chip_x0(22) - 4,
                304, 476, 352, t=3)
        ...
        damp_select = tune_page & (
            selected_dvi_q == RezoUISpec.TARGET_DAMP) & self.outline(
        ...
        layout_select = bands_page & (
            selected_dvi_q == RezoUISpec.TARGET_BAND_LAYOUT) & self.outline(
        ...
        mode_select = home_page & (
            selected_dvi_q == RezoUISpec.TARGET_MODE) & self.outline(
        ...
        filter_type_select = filter_page & (
            selected_dvi_q == RezoUISpec.TARGET_FILTER_TYPE) & self.outline(
        ...
        for destination in range(5):
            ...
            row_decode_y0 = row_y - (3)
            row_decode_y1 = row_y + (23)
        ...
        m.d.comb += [
            ...
            filter_cv_target_pre.eq(RezoUISpec.TARGET_FILTER_CV_BASE +
                                    filter_cv_index),
        ...
        filter_cv_select_signals.append(
            filter_cv_outer & (selected_dvi_q == filter_cv_target_q) &
        ...
        preset_select_signals.append(
            ...
            (selected_dvi_q == RezoUISpec.TARGET_PRESET) &
        ...
        selected_band = (
            ...
            RezoUISpec.TARGET_BAND_BASE + band_index_q)
        feedback_band_selected = (
            band_selected_target_value_q ==
            RezoUISpec.TARGET_FEEDBACK_SEND_BASE + band_index_q)
        enable_band_selected = (
            band_selected_target_value_q ==
            RezoUISpec.TARGET_BAND_ENABLE_BASE + band_index_q)
        frequency_band_selected = (
            band_selected_target_value_q ==
            RezoUISpec.TARGET_BAND_FREQ_BASE + band_index_q)
        bands_edit_outline = (
            band_active_value_q & band_select_x_q &
            ((enable_band_selected &
              (band_y_value_q >= bands_enable_y0 - 5) &
        ...
        # The row decoder and selected lane values are each registered before
        # endpoint arithmetic. Keep the renderer's established one-pixel
        # geometry prefetch; the added stage delays coordinates and values
        # together rather than changing their relative phase.
        ...
        input_clip_selected = Array(self.input_clips)[input_index]
        ...
        # block-RAM row decoder at high utilisation.
        input_targets = Array(
            Const(RezoUISpec.TARGET_INPUT_BASE + n * 3, 7)
            for n in range(4))
        input_target_selected = input_targets[input_index]

        # Isolate the synchronous row ROM and its 4:1 lane muxes from the
        # endpoint arithmetic. This shared two-stage shape is deliberately
        # identical in REZO, REZOMO, and STREZO.
        ...
        m.d.dvi += [
            ...
            input_clip_lane_q.eq(input_clip_selected),
        ...
        m.d.dvi += [
            ...
            input_clip_q.eq(input_clip_lane_q),
        ...
        input_fill_q0 = input_visible & Mux(
            ...
            Mux(~input_depth_negative_q,
                self.rect(input_x_value_q, input_local_value_q,
                          440,
                          66,
                          input_depth_end_q, 82),
                self.rect(input_x_value_q, input_local_value_q,
                          input_depth_end_q, 66,
                          440,
                          82)),
            self.rect(input_x_value_q, input_local_value_q,
        ...
        input_audio_meter_y0 = 50
        input_audio_meter_y1 = 52
        input_cv_meter_y0 = 82
        input_cv_meter_y1 = 84
        ...
        m.d.comb += [
            ...
            group_selected_valid_pre.eq(
                (selected_dvi_q >= RezoUISpec.TARGET_GROUP_BASE) &
                (selected_dvi_q < RezoUISpec.TARGET_GROUP_BASE +
                 RezoCore.N_BANDS)),
        ]
        ...
        group_select_signals.append(
            group_page & group_selected_valid & self.outline(
                x, y, group_selected_x - 5, 290,
                group_selected_x + 23, 470, t=3))
        ...
        output_col_active = output_col_rport.data[4] & ~(
            self.filter_mode & (output_source == 4))
        ...
        m.d.comb += [
            output_target.eq(RezoUISpec.TARGET_OUTPUT_BASE + output_source +
                             output_row + (output_row << 2)),
            output_select.eq(
                (output_outline_q &
                 (selected_dvi_q == output_target_q)) |
                output_header_select_q),
            output_header_row.eq(
                selected_dvi_q - RezoUISpec.TARGET_OUTPUT_ROW_BASE),
            output_header_col.eq(
                selected_dvi_q - RezoUISpec.TARGET_OUTPUT_COL_BASE),
            output_header_row_target.eq(
                (selected_dvi_q >= RezoUISpec.TARGET_OUTPUT_ROW_BASE) &
                (selected_dvi_q < RezoUISpec.TARGET_OUTPUT_ROW_BASE + 4)),
            output_header_col_target.eq(
                (selected_dvi_q >= RezoUISpec.TARGET_OUTPUT_COL_BASE) &
                (selected_dvi_q < RezoUISpec.TARGET_OUTPUT_COL_BASE + 4)),
            output_header_select.eq(output_header_selection(
                ...
                dry_selected=(
                    selected_dvi_q == RezoUISpec.TARGET_OUTPUT_DRY_COL),
        ...
        m.d.comb += preset_group_select.eq(
            ...
            ~self.editing & self.outline(
                ...
                NATIVE_PAGE_HEADER_SELECT_Y1, t=3))
        bank_control_y0s = (
            compact_main_control_y0s[:3])
        bank_panel_bounds = (
            tuple((row_y0 - 2, row_y0 + 18)
                  for row_y0 in bank_control_y0s))
        drive_select = (
            bank_page &
            (selected_dvi_q == RezoUISpec.TARGET_DRIVE) &
        ...
        tune_feedback_select = (
            tune_page &
            (selected_dvi_q == RezoUISpec.TARGET_FEEDBACK) & self.outline(x, y, tune_panel_x0,
                             NATIVE_FEEDBACK_AMOUNT_Y0 - 4 + tune_y_shift,
                             tune_panel_x1,
                             NATIVE_FEEDBACK_AMOUNT_Y0 + 20 + tune_y_shift,
                             t=3))
        res_select = ((bank_page & (selected_dvi_q == RezoUISpec.TARGET_RESONANCE)) |
                      (tune_page & (selected_dvi_q == RezoUISpec.TARGET_LIMIT_CAP))) & (
            (bank_page & self.outline(
                x, y, control_panel_x0,
                bank_panel_bounds[1][0], control_panel_x1,
                bank_panel_bounds[1][1], t=3)) |
            (tune_page & self.outline(x, y, tune_panel_x0,
        ...
        fb_select = (
            bank_page &
            (selected_dvi_q == RezoUISpec.TARGET_FEEDBACK) &
            self.outline(x, y, control_panel_x0,
        ...
        filter_freq_select = filter_page & (selected_dvi_q == RezoUISpec.TARGET_FILTER_CUTOFF) & self.outline(
            x, y, control_panel_x0, filter_panel_bounds[0][0], control_panel_x1,
            filter_panel_bounds[0][1], t=3)
        filter_slope_select = filter_page & (selected_dvi_q == RezoUISpec.TARGET_FILTER_SLOPE) & self.outline(
        ...
        m.d.dvi += [
            ...
            geometry_panel_q0.eq(preset_chip | filter_type_chip | mode_chip |
                                 palette_chip | save_default_chip | row_dry_chip |
                                 layout_chip |
        ...
        m.d.dvi += [
            ...
            line_q.eq(geometry_line_q0 | pager_line | input_line_q0 |
                      ...
                      input_bus_meter_zero_db_q0),
            mod_q.eq(geometry_mod_q0 | input_clip_q0 |
                     output_meter_clip_q0 | input_bus_meter_clip_q0),
            panel_q.eq(geometry_panel_q0 | input_panel_q0 | group_cell_q0 |
                       output_cell_q0 | filter_cv_panel_q0 |
                       output_meter_panel_q0 | input_bus_meter_panel_q0),
            background_q.eq(
                arc_background),
...
class RezoBeamTop(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        if cpu_firmware_path is None:
            raise ValueError("REZO production images require CPU firmware")
        ...
        try:
            ...
            from cpu_control import RezoCpuControlPlane
        m.submodules.cpu_control = cpu_control = RezoCpuControlPlane(
        ...
        # The combined-input view deliberately floors the roughly -51 dBFS
        # electrical idle level observed from a connected, muted source. This
        # is display-only: audio below the meter floor remains untouched.
        input_bus_db_init = [

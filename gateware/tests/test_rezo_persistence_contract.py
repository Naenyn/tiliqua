...
@pytest.mark.parametrize("journal_type", LEGACY_V1_JOURNALS)
def test_version_one_record_loads_with_current_tail_defaults(journal_type):
    slot = 4
    ...
    tail = (0x1357, 0x2468)
    record = make_record(
        journal_type, old_words, generation=5,
        version=journal_type.LEGACY_VERSION)
    contents = {base + n: byte for n, byte in enumerate(record)}
    run_boot(
        journal_type, contents, old_words + list(tail), slot=slot,
        state_words=6,

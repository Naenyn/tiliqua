...
from top.rezo.strezo_persistence import RezoStateJournal, SPIFlashTransfer
...
def make_record(words, generation=1, corrupt_at=None,
    ...
    payload = b"".join(int(word & 0xffff).to_bytes(2, "little")
                       for word in words)
    header = (RezoStateJournal.MAGIC.to_bytes(4, "little") +
...
def test_version_one_record_loads_through_both_migration_tails():
    ...
    record = make_record(old_words, generation=6,
                         version=RezoStateJournal.OLDEST_VERSION)
    contents = {base + n: byte for n, byte in enumerate(record)}
    run_boot(
        contents, old_words + list(oldest_tail), slot=slot, state_words=6,
        journal_kwargs={
